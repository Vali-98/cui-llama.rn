...
public class RNLlama implements LifecycleEventListener {
  ...
  private void persistUriPermission(String modelPath) {
  ...
  public void getCpuFeatures(Promise promise) {
    Future<WritableMap> future = executorService.submit(() -> {
      WritableMap result = Arguments.createMap();
    ...
    tasks.put(future, "getCpuFeatures");

    executorService.execute(() -> {
      try {
        WritableMap result = future.get(); // Waits for computation
        promise.resolve(result);
      } catch (Exception e) {
