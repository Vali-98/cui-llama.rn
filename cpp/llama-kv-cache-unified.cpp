    ...
    auto ctx_for_buft = [&](lm_ggml_backend_buffer_type_t buft) -> lm_ggml_context * {
        ...
        if (it == ctx_map.end()) {
            lm_ggml_init_params params = {
                /*.mem_size   =*/ size_t(2u*(1 + n_stream)*n_layer_cache*lm_ggml_tensor_overhead()),
                /*.mem_buffer =*/ NULL,
                /*.no_alloc   =*/ true,
            };
...
lm_ggml_tensor * llama_kv_cache_unified::get_k(lm_ggml_context * ctx, int32_t il, uint32_t n_kv, const slot_info & sinfo) const {
...
lm_ggml_tensor * llama_kv_cache_unified::get_v(lm_ggml_context * ctx, int32_t il, uint32_t n_kv, const slot_info & sinfo) const {
    const int32_t ikv = map_layer_ids.at(il);
...
lm_ggml_tensor * llama_kv_cache_unified::build_input_k_idxs(lm_ggml_context * ctx, const llama_ubatch & ubatch) const {
...
lm_ggml_tensor * llama_kv_cache_unified::build_input_v_idxs(lm_ggml_context * ctx, const llama_ubatch & ubatch) const {
    const uint32_t n_tokens = ubatch.n_tokens;
...
void llama_kv_cache_unified::set_input_k_idxs(lm_ggml_tensor * dst, const llama_ubatch * ubatch, const slot_info & sinfo) const {
    if (!supports_set_rows) {
        return;
    }
...
void llama_kv_cache_unified::set_input_v_idxs(lm_ggml_tensor * dst, const llama_ubatch * ubatch, const slot_info & sinfo) const {
    if (!supports_set_rows) {
        return;
    }
...
lm_ggml_cgraph * llama_kv_cache_unified::build_graph_shift(llm_graph_result * res, llama_context * lctx) const {
...
lm_ggml_cgraph * llama_kv_cache_unified::build_graph_defrag(
    ...
    LM_GGML_ASSERT(n_stream == 1 && "n_stream > 1 does not support defrag");
