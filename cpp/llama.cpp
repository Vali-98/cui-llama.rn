...
static struct lm_ggml_tensor * llm_build_inp_embd(
    ...
    if (ubatch.token) {
        lctx.inp_tokens = lm_ggml_new_tensor_1d(ctx, LM_GGML_TYPE_I32, ubatch.n_tokens);
...
static struct lm_ggml_tensor * llm_build_moe_ffn(
       ...
       struct llama_context & lctx,
         ...
         struct lm_ggml_tensor * exp_probs_b,
                    int64_t   n_expert,
...
llama_expert_gating_func_type gating_op,
    ...
    switch (gating_op) {
        case LLAMA_EXPERT_GATING_FUNC_TYPE_SOFTMAX:
            {
                probs = lm_ggml_soft_max(ctx, logits); // [n_expert, n_tokens]
            } break;
        case LLAMA_EXPERT_GATING_FUNC_TYPE_SIGMOID:
            {
                probs = lm_ggml_sigmoid(ctx, logits); // [n_expert, n_tokens]
            } break;
        default:
    ...
    if (exp_probs_b != nullptr) {
        selection_probs = lm_ggml_add(ctx, probs, exp_probs_b);
        cb(selection_probs, "ffn_moe_probs_biased", il);
    }

    // select experts
    lm_ggml_tensor * selected_experts = lm_ggml_top_k(ctx, selection_probs, n_expert_used); // [n_expert_used, n_tokens]
    ...
    struct lm_ggml_cgraph * build_cohere2() {
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            // self-attention
            {
                ...
                if (is_sliding) {
                    Qcur = lm_ggml_rope_ext(ctx0, lm_ggml_reshape_3d(ctx0, Qcur, n_embd_head, n_head, n_tokens), inp_pos, rope_factors,
                                        n_rot, rope_type, n_ctx_orig, freq_base, freq_scale, ext_factor, attn_factor,
                                        beta_fast, beta_slow);
                    cb(Qcur, "Qcur", il);

                    Kcur = lm_ggml_rope_ext(ctx0, lm_ggml_reshape_3d(ctx0, Kcur, n_embd_head, n_head_kv, n_tokens), inp_pos,
