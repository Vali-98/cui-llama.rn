...
lm_ggml_tensor * llm_build_gemma3n_iswa::get_per_layer_inputs() {
    ...
    if (ubatch.token) {
        ...
        lm_ggml_tensor * padding_q = lm_ggml_view_1d(ctx0, model.tok_embd_per_layer, embd_size, 0);
        lm_ggml_tensor * padding_f32 = lm_ggml_new_tensor_1d(ctx0, LM_GGML_TYPE_F32, embd_size);
        inp_per_layer = lm_ggml_cpy(ctx0, padding_q, padding_f32);
