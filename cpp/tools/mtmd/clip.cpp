...
struct clip_graph {
    ...
    lm_ggml_cgraph * build_qwen3vl() {
        ...
        lm_ggml_tensor * deepstack_features = nullptr;
        ...
        for (int il = 0; il < n_layer; il++) {
            ...
            if (layer.has_deepstack()) {
                lm_ggml_tensor * feat = lm_ggml_reshape_3d(ctx0, cur, n_embd * merge_factor, n_pos / merge_factor, batch_size);
                ...
                if(!deepstack_features) {
                    ...
                    deepstack_features = lm_ggml_concat(ctx0, deepstack_features, feat, 0);
                }
            }
    ...
    lm_ggml_cgraph * build_llava() {
        ...
        // glm projector
            ...
            {
                embeddings = lm_ggml_concat(ctx0, model.mm_boi, embeddings, 1); // BOI
                embeddings = lm_ggml_concat(ctx0, embeddings, model.mm_eoi, 1); // EOI
            }
        }
    ...
    lm_ggml_cgraph * build_cogvlm() {
        ...
        lm_ggml_tensor * h_to_4h = lm_ggml_mul_mat(ctx0, model.mm_h_to_4h_w, cur);

        // Branch 2: multiply with mm_gate_w
        lm_ggml_tensor * gate = lm_ggml_mul_mat(ctx0, model.mm_gate_w, cur);

        // Apply silu
        gate = lm_ggml_swiglu_split(ctx0, gate, h_to_4h);

        // Apply mm_4h_to_h_w
        cur = lm_ggml_mul_mat(ctx0, model.mm_4h_to_h_w, gate);
