...
struct lm_ggml_hexagon_opqueue {
    ...
    lm_ggml_hexagon_opqueue(lm_ggml_hexagon_session *sess, size_t batch_size, size_t depth) {
        ...
        if (opt_verbose) {
            LM_GGML_LOG_INFO("ggml-hex: %s allocated op-queue : batch-size %zu depth %zu shm-size %zu shm-block-size %zu\n",
    ...
    void pop(htp_opbatch_rsp rsp, dspqueue_buffer dbuf) {
        ...
        if (opt_profile && rsp.n_ops > 0) {
            ...
            uint64_t batch_usec = lm_ggml_time_us() - start_usec[rsp.id];
            uint32_t htp_usec   = 0;
...
void lm_ggml_hexagon_session::allocate(int dev_id) noexcept(false) {
    ...
    this->op_batch = new lm_ggml_hexagon_opbatch(this, opt_opbatch, opt_vmem);
