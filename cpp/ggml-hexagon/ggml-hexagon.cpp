    ...
    switch (t->op) {
        ...
        case LM_GGML_OP_UNARY:
            if (lm_ggml_get_unary_op(t) == LM_GGML_UNARY_OP_SILU) {
                ...
                supported = true;
            } else if (lm_ggml_get_unary_op(t) == LM_GGML_UNARY_OP_GELU) {
...
static inline bool op_reuse_src1(const lm_ggml_tensor * op1, const lm_ggml_tensor * op0) {
    return (op0 && op0->src[1] == op1->src[1] && lm_ggml_is_quantized(op0->src[0]->type) && lm_ggml_is_quantized(op1->src[1]->type));
}
...
static bool lm_ggml_backend_hexagon_device_supports_op(lm_ggml_backend_dev_t dev, const struct lm_ggml_tensor * op) {
    ...
    switch (op->op) {
        ...
        case LM_GGML_OP_UNARY:
            {
                const auto unary_op = lm_ggml_get_unary_op(op);
                if (unary_op == LM_GGML_UNARY_OP_SILU || unary_op == LM_GGML_UNARY_OP_GELU) {
